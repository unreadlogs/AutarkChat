...
function AutarkLogo({ size = 32 }: { size?: number }) {
  return (
    <svg
      viewBox="0 0 775 740"
...
export default function NotFound() {
  return (
    <div className="flex min-h-dvh flex-col bg-[var(--canvas)]">
      {/* Top bar */}
      <header className="flex h-14 items-center border-b border-[var(--hairline-soft)] px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2.5 text-[var(--ink)]">
          <AutarkLogo size={22} />
          <span className="text-[13px] font-semibold tracking-tight">autarkchat</span>
        </Link>
      </header>
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6 text-center">
        {/* Eyebrow */}
        <p
          className="mb-6 text-[13px] font-medium uppercase tracking-[0.35px] text-[var(--stone-t)]"
        ...
        {/* Display numeral */}
        <div
          className="mb-2 select-none font-semibold leading-none text-[var(--hairline-soft)]"
          style={{
        ...
        {/* Hairline divider */}
        <div className="my-8 h-px w-16 bg-[var(--hairline-soft)]" />

        {/* Headline */}
        <h1
          className="mb-3 text-[var(--ink)]"
          style={{ fontSize: "32px", fontWeight: 400, letterSpacing: "-0.9px", lineHeight: 1.1 }}
        >
          Page not found
        </h1>
        ...
        {/* Sub-copy */}
        <p
          className="mb-10 max-w-sm text-[var(--graphite)]"
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          <Link
            href="/chat"
            className="flex items-center gap-2 rounded-full bg-[var(--ink)] px-5 py-2.5 text-[14px] font-semibold text-[var(--canvas)] transition-opacity hover:opacity-80"
          >
            Go to Chat
          </Link>
          <Link
            href="/docs"
            className="flex items-center gap-2 rounded-full border border-[var(--ink)] bg-[var(--canvas)] px-5 py-2.5 text-[14px] font-semibold text-[var(--ink)] transition-opacity hover:opacity-70"
          >
            Documentation
          </Link>
        </div>
      ...
      {/* Footer */}
      <footer className="flex h-14 items-center justify-center border-t border-[var(--hairline-soft)] px-6">
        <p className="text-[12px] text-[var(--stone-t)]">
          autarkchat · <Link href="/docs" className="hover:text-[var(--ink)] transition-colors">docs</Link> · <Link href="/chat" className="hover:text-[var(--ink)] transition-colors">chat</Link>
        </p>
      </footer>
    </div>
  );
